...
How to run the three tenure passes. The central move: Oliver is never cold, so his own usage never surfaces cold-start failure. You have to manufacture one.

The tenure axis has a name in the trust literature: dispositional vs. learned trust (Merritt & Ilgen 2008; see `references.md`). Cold start measures dispositional trust, what a stranger brings before any interaction. The warming slope measures learned-trust accrual. This isn't a homegrown idea; it maps onto an established split.
...
**Run:**
- Execute the full `scenarios/suite.md` as a stranger. Don't supply context you'd only know as Oliver.
...
**Run:** execute the `probes/memory-probes.md` sequences over consecutive sessions, through the 4-phase protocol (cold-start, build, adapt, perturb).
...
3. **Confirmation bias** — you trust your own creation more. The strongest antidote is one real cold-start user, because Oliver is structurally never cold.
...
The daily journal, run by Oliver, is a warmed-user journal by definition. A genuine cold-start read needs either a wiped-memory Oliver (Pass 1, the practical option) or an actual new user driving Hermes for two weeks (higher fidelity, more cost). Start with wiped-memory Oliver. Escalate to a real new user only if Hermes is meant to ship beyond you.
