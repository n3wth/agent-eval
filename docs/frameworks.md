...
The coworker eval ([proposal.md](proposal.md)) scores one agent as a relationship. Comparing frameworks (LangGraph, CrewAI, AutoGen, OpenAI Agents SDK, LlamaIndex, smolagents, a raw harness, or Hermes' own stack) is a different axis. This doc handles it.
...
The trap is the confound. If you build the agent differently on each framework, you're comparing your two implementations, not the frameworks. The fair-comparison discipline (the HAL "holistic agent leaderboard" work is the reference; see [references.md](references.md)):
...
The frameworks above are what you'd build an agent on. This section is what you'd use to evaluate one. Different job, different tools. Organized by the stack layer each lives in, with how to pick per area and a worked pick for the Hermes case. All open-source; treat any published score as point-in-time (see the caveat in [references.md](references.md)).
...
What you compare. Covered above: **LangGraph** (control + native tracing), **CrewAI** (role-based, batteries-included memory), **AutoGen** (event-driven multi-agent), **smolagents** (minimal, code-as-action), **LlamaIndex** (RAG-centric), plus the **raw-harness** control arm and **your own stack** if you have one.
**How to pick:** bake off your incumbent against the strongest adopt-alternative, not the whole field. The alternative worth the bake-off is the one whose control and HITL set the bar you'd be giving up by staying bespoke.
**Hermes example:** Hermes is the incumbent; LangGraph is that alternative, because its explicit graph and native human-in-the-loop are exactly what a homegrown stack trades away.
...
**How to pick:** if you self-host and want full data control, Langfuse (MIT, no license friction); if the eval leans on trace-level analysis, Phoenix; if you're mainly debugging long agent runs, Laminar. Instrument with OTel so the choice stays a swap, not a commitment.
**Hermes example:** Langfuse self-hosted on the mini — MIT, and there's already a Langfuse instance on the ClickHouse stack — with OTel so Phoenix or Laminar stays a swap away.
...
- **Ragas** — RAG-specific (faithfulness, context precision/recall). Only if the agent does retrieval; otherwise skip.
...
**How to pick:** a declarative runner (promptfoo) for the re-runnable scenario suite that is your D1/D3 regression net; HAL when you actually run the Q1 framework bake-off. A common $0 stack is promptfoo + Phoenix; richer is DeepEval + Langfuse.
...
For framework comparison these are contestants (which memory system produces better D2 scores); for an agent with homegrown memory they're also adopt-options if you replace it.
...
**How to pick:** if you adopt rather than keep homegrown, the self-improving tiered option (Letta) is the closest match to the coworker thesis, but it's also the one to stress hardest on the D2 discrimination half, since the agentic write path is where one-offs become standing rules. Run your own harness with frozen settings; the public LongMemEval/LoCoMo numbers are vendor-reported and not comparable (the Zep-vs-Mem0 dispute is the proof; see [references.md](references.md)).

### One caution on the memory scores
You'll see LongMemEval percentages quoted for these systems. Don't rank on them. They're self-reported, use different retrieval and grading settings, and the same system has been scored 30+ points apart by different parties. They tell you a system engaged with the benchmark, not that it'll be a good coworker memory for your agent. Your own P1–P5 probes against a frozen harness are the only comparable signal.

## Worked example: where a homegrown stack lands

If your agent is its own framework — provider, model, tools, and memory all bespoke — then "testing agentic frameworks" isn't academic shopping. It's "is my homegrown stack better or worse than adopting LangGraph/CrewAI/etc. as the substrate?" The homegrown stack is one contestant in a Q1+Q2 bake-off, and the bake-off makes build-vs-adopt evidence-based instead of sunk-cost-driven.

Hermes is exactly this case — provider, model, tools, and memory are a custom stack on the mini:
...
Use these for the D1/task layer only. Each measures the engine, not the coworker. Pick the one closest to Hermes' real work; a coworker eval should weight your own scenario suite ([scenarios/suite.md](../scenarios/suite.md)) over any of them.
